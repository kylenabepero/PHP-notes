...
if (isset($_POST['submitted']))
{
    ...
    {
        // get file name
        $fileName = str_replace(' ', '_', $_FILES['image']['name'][$key]);
        ...
        // check size
        if ($_FILES['image']['size'][$key] > 0 && $_FILES['image']['size'][$key] <= $maxFileSize)
        {
            $sizeOK = true;
        }
        ...
        {
            if ($filetype == $_FILES['image']['type'][$key])
            {
                $typeOK = true;
                break;
            }
        ...
        else
        {
            ...
            {
                case 0:
                    $success = move_uploaded_file($_FILES['image']['tmp_name'][$key], $uploadDir.$fileName);
                    if ($success)
                    {
                        $r[] = "$fileName uploaded!";
                    }
